...
from rlpd.data.d4rl_datasets import D4RLDataset, MetaworldDataset
...
flags.DEFINE_string("env_name", "reach", "Metaworld env name")
...
def main(_):
    ...
    import pdb; pdb.set_trace()
    expert_data_path = os.path.join(log_dir, "experts.hdf")
    generate_expert_data(env_str, dmc_env, expert_data_path, 100, False, False)
    ds = MetaworldDataset(expert_data_path)

    eval_env = DMCWrapper(LowDimMetaworldEnv(env_str))
